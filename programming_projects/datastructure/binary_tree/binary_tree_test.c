...
void enqueue(struct Node_queue **head, struct Node_queue **tail,
             struct Node *data) {
  struct Node_queue *new_node =
      (struct Node_queue *)malloc(sizeof(struct Node_queue));
  (*new_node).data = data;
  (*new_node).next = NULL;
...
int is_lesser(struct Node *root, int data) {
  ...
  if ((*root).data <= data && is_lesser((*root).left, data) == 1 &&
      is_lesser((*root).right, data) == 1) {
    return 1;
  } else {
    return 0;
  }
...
int is_greater(struct Node *root, int data) {
  ...
  if ((*root).data > data && is_greater((*root).left, data) == 1 &&
...
int is_search_tree(struct Node *root) {
  ...
  int data = (*root).data;
  printf("%d\n", data);
  if (is_lesser((*root).left, data) == 1 &&
      is_greater((*root).right, data) == 1 && is_search_tree((*root).left) &&
      is_search_tree((*root).right)) {
    return 1;
  } else {
    return 0;
  }
...
int is_search_tree_fast(struct Node *root, int min, int max) {
  ...
  if (data >= min && data < max &&
      is_search_tree_fast((*root).left, INT_MIN, (*root).data) == 1 &&
      is_search_tree_fast((*root).right, (*root).data, INT_MAX) == 1) {
    return 1;
  } else {
    return 0;
  }
...
void inorder_enqueue(struct Node_queue **head, struct Node_queue **tail,
                     struct Node *root) {
  if (root == NULL) {
    return;
  }
...
int elements[] = {
    326, 275, 840, 297,  767, 659, 191, 592, 560, 281, 670,  887, 359, 796, 313,
...
min_value, max_value); int min_value_recursion = find_min_recursion(root); int
max_value_recursion = find_max_recursion(root); printf("\n\nThe minimum value of
this tree is %d and the maximum value is %d\n", min_value_recursion,
max_value_recursion); int height = find_height(root); printf("\nThe height of
this thang is %d\n", height); for(int i = 0; i < 100; i++){ search(root, i );
    }
}*/
int main(void) {
  struct Node *root = NULL;
