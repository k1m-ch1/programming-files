my_string="""1914 - Born in Cresco, Iowa
...
1944 - Rejects a 100percent salary increase from Dupont, leaves behind his pregnant wife, and flies to Mexico to head a new plant pathology program. Over the next 16 years, his team breeds 6,000 different strains of disease resistent wheat - including different varieties for each major climate on Earth.
1945 - Discovers a way to grown wheat twice each season, doubling wheat yields
1953 - crosses a short, sturdy dwarf breed of wheat with a high-yeidling American breed, creating a strain that responds well to fertilizer. It goes on to provide 95percent of Mexico's wheat.
...
def format_year(my_str):
  return "<strong>" + my_str[0:4]+"</strong>" + my_str[4:]

get_list_element = lambda x: ["<li>"+format_year(y)+"</li>" for y in x.split('\n')]

list_element = get_list_element(my_string)
